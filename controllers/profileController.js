...
module.exports.bookAppointment = async (req, res) => {
    ...
    if (user_appointments.length > 0) {
        ...
        if (hospital_appointments.length > 0) {
            ...
            if (remaining_mins > 0) {
                return res.status(200).json({ error: `Wait for ${remaining_mins} minute(s), there is an ongoing appointment booking in this hospital` })
            }
        }
        ...
        if (hospital_appointments.length > 0) {
            ...
            let remaining_mins = Math.trunc((last_hospital_appointment_expiry - Date.now()) / (1000 * 60))
...
module.exports.userAppointments = async (req, res) => {
    ...
    try {
        const data = await AppointmentModel.find({ user: user_id }).populate('hospital', 'user')
